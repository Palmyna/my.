...
import { createPhysicalCopiesService, createPhysicalCopy, deletePhysicalCopy, listPhysicalCopies, PhysicalCopiesError, updatePhysicalCopyName } from './physical-copies'
import { getSupabaseClient } from './supabase'

vi.mock('./supabase', () => ({ getSupabaseClient: vi.fn() }))
const copy = { id: 'copy-id', name: null, created_at: '2026-09-23T00:00:00Z' }
...
test('list filters owner and variant and orders by creation then ID', async () => {
  const mock = setup([{ ...copy, note: 'not exposed' }])
  ...
  expect(mock.select).toHaveBeenCalledExactlyOnceWith('id,name,created_at')
...
test.each(['', '  ', ' Mon exemplaire '])('create exactly one copy, normalize optional name %j', async name => {
  ...
  expect(mock.insert).toHaveBeenCalledExactlyOnceWith({ variant_id: 42, name: name.trim() || null })
...
test('create without name persists NULL', async () => {
  ...
  expect(mock.insert).toHaveBeenCalledExactlyOnceWith({ variant_id: 42, name: null })
})

test.each([' Nouveau nom ', '', '\t\n'])('update changes only name, supports clearing %j', async name => {
  const mock = setup()
  await mock.service.updateName(copy.id, name)
  expect(mock.update).toHaveBeenCalledExactlyOnceWith({ name: name.trim() || null })
...
test.each(['updateName', 'delete'] as const)('zero rows for %s never reports success', async operation => {
...
test.each([
  ...
  for (const operation of [() => mock.service.list('owner', 42), () => mock.service.create(42),
    () => mock.service.updateName(copy.id, ''), () => mock.service.delete(copy.id)]) {
...
test.each([null, {}, [{ ...copy, name: 3 }], [{ ...copy, created_at: 'invalid' }], [{ ...copy, id: '' }]])('invalid listing %j fails closed', async data => {
...
test('all runtime entry points use configured client and fail safely without it', async () => {
  ...
  await updatePhysicalCopyName(copy.id, 'Nom')
  ...
  for (const operation of [() => createPhysicalCopy(42, ''), () => updatePhysicalCopyName(copy.id, ''),
