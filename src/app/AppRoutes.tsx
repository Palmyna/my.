...
import { AuthProblemPage, ConfirmEmailPage, DashboardPage, ForgotPasswordPage, HomePage, LoginPage, MfaChallengePage, MfaEnrollPage, ResetPasswordPage, SignupPage } from '../features/auth/AuthPages'
...
export function AppRoutes() {
  ...
  const target = auth.status === 'mfa_enrollment_required' ? '/auth/mfa/enroll'
    ...
    : auth.status === 'password_reset_required' ? '/reset-password'
    : auth.isAuthorized ? '/dashboard' : null
  ...
  return (
    <Routes>
      ...
      <Route path="/dashboard" element={auth.isAuthorized ? <DashboardPage /> : <Navigate to="/login" replace />} />
      <Route path="*" element={<AuthLayout title="Cette page n’existe pas."><Link className="button primary" to="/">Revenir à l’accueil</Link></AuthLayout>} />
    </Routes>
  )
}
