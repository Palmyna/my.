...
import { dateOrigin, hash } from './model.ts'
...
export function report(catalogue: Catalogue, plan: Plan, source: Snapshot, overridesHash: string, mode: string, started: string) {
  ...
  const variantDates = { variant: 0, card: 0, product: 0, set: 0, override: 0, unknown: 0 }
  for (const row of plan.rows.catalog_variants) variantDates[dateOrigin.parse(row.date_origin)]++
  ...
  const diagnostics = [...catalogue.diagnostics, ...plan.diagnostics]
  ...
  return {
    ...
    pokemon_reference: plan.pokemonReference,
    ...
    diagnostic_counts: diagnosticCounts, diagnostics,
...
export function printReport(value: Report, file: string): void {
  ...
  console.log('Dates des variantes :', value.variant_dates, 'Sans date :', value.catalogue.variants_without_date)
