...
La [sonde locale du 13 septembre 2026](reports/2026-09-13-PHASE4B1-REAUTH.md) vérifie ces primitives avec `@supabase/supabase-js` **2.115.0** et Auth **v2.196.0** : `signInWithPassword` contrôle le mot de passe puis remplace la session du client par une nouvelle session `aal1`, avec un nouveau `session_id` et l'événement `SIGNED_IN`. Un mauvais mot de passe conserve la session précédente. Sur le facteur TOTP vérifié existant, `mfa.challenge` crée un nouveau challenge et `mfa.verify` élève cette nouvelle session en `aal2`, renouvelle ses tokens et émet `MFA_CHALLENGE_VERIFIED`. Aucun remplacement de facteur n'est nécessaire.

Les claims Auth `amr` portent les méthodes et leurs horodatages ; un refresh conserve ceux des vérifications précédentes. Ils ne portent pas une autorisation MY. à usage unique liée à une action. Leur lecture dans le navigateur ne remplace pas une validation autoritative. Aucun état de fraîcheur, délai local ou stockage supplémentaire n'est introduit tant que le contrôle des opérations reste bloqué.
...
**Blocage confirmé avant implémentation :** sur un compte temporaire local, `PUT /auth/v1/user` accepte un nouveau mot de passe et une demande de changement d'email avec la seule session `aal2` déjà ouverte, sans nouvelle vérification des facteurs entre ces opérations. Le nouvel email reste en attente des confirmations Auth. Le [code Auth v2.196.0 de cet endpoint](https://github.com/supabase/auth/blob/v2.196.0/internal/api/user.go) contrôle le niveau MFA, sans exiger de nouveau TOTP propre à l'action.

La RLS applicative ne contrôle pas ces endpoints. Un état React temporaire, même correctement invalidé, ou un endpoint MY. protégé laisserait l'accès natif contournable. L'implémentation du socle commun est donc arrêtée sur cette limite réelle, sans affaiblir l'exigence validée. La reprise nécessite de définir et valider un contrôle Auth/serveur qui impose les deux facteurs frais sur tous les chemins accessibles des opérations concernées, en préservant le recovery. Aucune architecture de remplacement ni protection supplémentaire n'est réputée livrée.
