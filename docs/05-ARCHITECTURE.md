...
Le périmètre fonctionnel et UX de `/profile` est cadré dans [01-FEATURES.md](01-FEATURES.md#profil-utilisateur) et [04-UX-UI.md](04-UX-UI.md#profil-utilisateur). La page finale reste à construire. Le contrat Auth des Phases 3A/3B est prolongé par le service et les callbacks de changement d'email décrits ci-dessous ; aucune suppression de compte ni opération de changement volontaire de mot de passe n'est encore livrée.
...
| Suppression future | Confirmation explicite + mot de passe actuel + TOTP frais + validation finale + opération serveur privilégiée contrôlée par MY. |
...
**Mot de passe — blocage local ciblé :** le SDK **2.115.0** accepte `current_password` et Auth **v2.196.0** sait l'exiger via `GOTRUE_SECURITY_UPDATE_PASSWORD_REQUIRE_CURRENT_PASSWORD=true`. En revanche, la CLI **2.116.0** du dépôt n'expose pas cette option dans son [schéma email](https://github.com/supabase/cli/blob/v2.116.0/packages/config/src/auth/email.ts) et ne la transmet pas au conteneur Auth dans son [lancement](https://github.com/supabase/cli/blob/v2.116.0/apps/cli/src/legacy/commands/start/services/gotrue.service.ts). Le conteneur et les appels directs confirment cette absence : une session `aal2` reçoit HTTP 200 sans mot de passe actuel et avec un mot de passe actuel incorrect. Les refus serveur exigés ne sont donc pas validés et l'opération volontaire n'est pas livrée. Aucune clé TOML inventée, modification du binaire ou reconfiguration Docker parallèle n'est introduite.
...
Avant production, activer et vérifier l'exigence **Require current password when changing password** décrite par la [documentation Auth](https://supabase.com/docs/guides/auth/password-security#require-current-password-when-changing-password), correspondant à `GOTRUE_SECURITY_UPDATE_PASSWORD_REQUIRE_CURRENT_PASSWORD=true`. Conserver **Secure email change** activé et valider les appels directs ainsi que le recovery. Aucun réglage cloud n'est appliqué ou présumé actif par cette tâche. Le lancement local reproductible capable d'activer l'exigence du mot de passe actuel reste à résoudre avant de livrer cette opération.
...
- la méthode finale de ré-authentification fraîche mot de passe + TOTP et la preuve côté serveur pour la suppression ;
- le lancement local reproductible permettant d'activer le mot de passe actuel obligatoire côté Auth, puis l'opération volontaire correspondante et sa validation ;
