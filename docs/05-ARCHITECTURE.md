...
MY. distingue développement et production. Les Phases 1 et 2, le catalogue, la préparation des préférences, la migration Auth 3A et la migration de suppression 4B.3 sont déployés dans Supabase Cloud : huit migrations, détaillées dans le [README](../README.md) et confirmées en lecture seule en [4D.3](reports/2026-09-15-PHASE4D3-CLOUD-CHECKPOINT.md). La fonction `delete-account` déjà déployée est validée sur des fixtures temporaires. Le pipeline refuse toujours toute base distante. Le staging et les futurs workflows de déploiement restent à cadrer.
