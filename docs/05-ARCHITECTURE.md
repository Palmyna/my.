...
Le périmètre fonctionnel et UX de `/profile` est cadré dans [01-FEATURES.md](01-FEATURES.md#profil-utilisateur) et [04-UX-UI.md](04-UX-UI.md#profil-utilisateur). La [Phase 4C](reports/2026-09-14-PHASE4C-PROFILE.md) livre `ProfilePage` : identité MY., date d'inscription, formulaire email et statut Authenticator. Elle utilise exclusivement `useAuth()`, sans second client, cache de données du compte ou requête Supabase dans React. Les états de saisie, copie, soumission et feedback restent locaux. Le service, le store et le callback email 4B.2 sont réutilisés sans modification. L'UX de suppression et le changement volontaire de mot de passe restent à terminer en 4D et au checkpoint cloud.
