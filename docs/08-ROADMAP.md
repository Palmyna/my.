...
La première liste de contenu est branchée sous l'overview, avec réorganisation propriétaire et gestion/consultation des exemplaires. La recherche serveur de variantes exactes et son service sont livrés ; l'interface d'ajout reste à réaliser. Les migrations Phase 6 sont appliquées et validées sur Supabase local jusqu'à 6C.2 ; elles restent non appliquées sur Supabase Cloud, en attente du checkpoint manuel de fin de phase. La phase n'est pas clôturée.
