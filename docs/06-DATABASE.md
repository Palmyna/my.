...
La suite DB complète officielle passe après migration : **858 assertions, 16 fichiers**, dont 60 assertions exemplaires et 78 mutations manuelles. `001_schema` attendait déjà la limite de note 6A.2 ; aucune correction nécessaire. Dans `013_physical_copies`, les noms système conservaient la collation `C` après conversion en `text`, tandis que les tableaux attendus utilisaient la collation par défaut : pgTAP échouait lors de la comparaison des enregistrements. Les projections de noms de colonnes/policies utilisent désormais explicitement `COLLATE "default"`, sans modifier le schéma ni désactiver les assertions. La syntaxe `VALUES (` du test a également été corrigée. Aucun frontend, service React Query, recherche ou UI d'ajout/retrait développé.
...
Contrat additif : anciens lecteurs/écritures inchangés. Une migration de retrait pourra supprimer uniquement cette fonction après retrait de ses consommateurs, sans restauration de données. Service livré en 6B.2, affichage branché en 6B.3 ; le service utilise désormais directement les types Supabase régénérés localement, sans overlay de schéma futur. La validation du JSONB reçu reste inchangée.
...
Une ligne représente un exemplaire physique individuel. Modèle appliqué localement depuis la migration 6A.2, en attente du checkpoint Cloud :
...
La [migration 6A.3](../supabase/migrations/20260923195220_phase6a3_reorder_collection_item.sql) est **appliquée sur Supabase local**, en attente du checkpoint Cloud. Elle ajoute deux contrats, sans changer les tables, les RLS ni les grants d'écriture directe :
