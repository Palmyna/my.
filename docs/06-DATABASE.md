...
La Phase 4 est cadrée et en cours d'implémentation. Les [protections des actions du compte](05-ARCHITECTURE.md#sécurité-des-actions-de-gestion-du-compte) distinguent email/mot de passe gérés par Auth et suppression renforcée côté serveur MY. Le changement d'email et le backend de suppression sont livrés localement. La migration [20260914102414_phase4b3_account_deletion.sql](../supabase/migrations/20260914102414_phase4b3_account_deletion.sql) ajoute le trigger privé de nettoyage et les restrictions RLS contre les JWT d'un compte supprimé, sans table ni FK supplémentaire. Elle porte le total local à **huit migrations**, sans déploiement cloud. La [Phase 4D.1](reports/2026-09-15-PHASE4D1-PASSWORD-PROFILE.md) livre le formulaire de mot de passe et le contrat Auth `password` / `current_password`, sans changement SQL, RLS ou migration. La [Phase 4D.2](reports/2026-09-15-PHASE4D2-ACCOUNT-DELETION-UX.md) intègre l'UX de suppression au backend existant, sans changement de schéma, RLS, types ou configuration. Le refus serveur du mot de passe actuel absent/incorrect reste à valider sur Cloud en 4D.3 ; les tests locaux de contrat ne démontrent pas cette garantie.
