...
**Réalisation Phase 4C :** trois sections sobres — Identité MY., Adresse email, Authenticator — reprennent le shell, Poppins et les styles existants, avec des contours discrets et des espacements réguliers. L'identifiant apparaît sous le libellé `MY.ID`, dans un input texte en lecture seule de hauteur standard, aligné avec le bouton de copie à droite ; les adresses peuvent revenir à la ligne. Les données absentes ont un message explicite, sans valeur de remplacement inventée. Les retours de copie et d'email sont locaux et accessibles. Le `h1` unique conserve le focus de navigation géré par `AppRoutes`. Le [rapport 4C](reports/2026-09-14-PHASE4C-PROFILE.md) consigne les contrôles responsive et clavier.
