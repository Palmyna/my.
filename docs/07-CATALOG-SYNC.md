...
Le **pipeline de synchronisation** reste volontairement local et protégé, tandis que le **catalogue résultant** existe aussi dans le cloud. Les restrictions des commandes ci-dessous ne remettent pas en cause cet état déployé. Les migrations ultérieures jusqu'à la Phase 5 sont également déployées dans le Cloud, selon les validations consignées dans le [README](../README.md#état-du-projet) : onze migrations cumulées. Elles ne modifient pas le périmètre local du pipeline.
...
La recherche applicative d'ajout 6C.2 utilise une [RPC dédiée](06-DATABASE.md#recherche-catalogue-pour-ajout--contrat-6c2), indépendante de `catalog:find`, de la recherche globale et du filtre interne. Elle reprend le matching et le score du moteur portable avec tests de parité, mais retourne chaque Variante éligible à 6C.1 séparément. Les cartes/sets inactifs et variantes inactives/non confirmées sont exclus ; `source_present` n'est pas exigé. La CLI et son chargement complet, réservés à la maintenance, restent inchangés.
...
Auth, gestion du compte, Dashboard, overview Collection, création personnalisée, renommage/suppression et backend de création automatique sont livrés hors du pipeline. Les interactions avec le contenu, la recherche UI, les pages catalogue, les préférences, la mise à jour automatique et le parcours utilisateur de partage restent futurs selon la [roadmap](08-ROADMAP.md) ; les notifications restent à cadrer. Le comportement de recherche globale, des pages catalogue et des préférences est validé dans les documents produit/UX ; son implémentation n'est pas ajoutée au pipeline. Langage, cache, JSON/Zod, identité, stamps, FR, Jumbo, numéros, dates inconnues, ordre, transaction, dry-run, hash et version sont implémentés et testés.
