...
Le **pipeline de synchronisation** reste volontairement local et protégé, tandis que le **catalogue résultant** existe aussi dans le cloud. Les restrictions des commandes ci-dessous ne remettent pas en cause cet état déployé. Les migrations des invariants de collections/préférences et du socle Auth 3A sont également déployées dans le cloud, selon le propriétaire : sept migrations cumulées. Le socle Auth 3A est validé localement et dans Supabase cloud ; il ne modifie pas le pipeline.
