...
insert into schema_fixture_users (id) values
  ('10000000-0000-0000-0000-000000000001'),
  ('10000000-0000-0000-0000-000000000002'),
  ('10000000-0000-0000-0000-000000000003'),
  ('10000000-0000-0000-0000-000000000004');
insert into auth.users (id) select id from schema_fixture_users;
...
select ok((select bool_and(public_id::text ~ '^MY-([23456789ABCDEFGHJKMNPQRSTUVWXYZ]{5}-){3}[23456789ABCDEFGHJKMNPQRSTUVWXYZ]{5}$') from profiles where id in (select id from schema_fixture_users)), 'Every fixture profile receives a valid canonical public ID');
select is((select count(distinct public_id) from profiles where id in (select id from schema_fixture_users)), 4::bigint, 'Generated fixture IDs differ');
select is((select count(*) from profiles where public_id = (select lower(public_id::text)::citext from profiles where id = '10000000-0000-0000-0000-000000000001')), 1::bigint, 'Case-insensitive lookup returns the same profile');
select throws_ok($$update profiles set public_id = lower(public_id::text) where id = '10000000-0000-0000-0000-000000000001'$$, '23514', 'public_id is immutable', 'Case-only update also forbidden');
select throws_ok($$update profiles set public_id = 'MY-22222-22222-22222-22222' where id = '10000000-0000-0000-0000-000000000001'$$, '23514', 'public_id is immutable', 'Changing public ID forbidden');
select throws_ok($$update profiles set public_id = null where id = '10000000-0000-0000-0000-000000000001'$$, '23514', 'public_id is immutable', 'Clearing public ID forbidden');
select lives_ok($$update profiles set public_id = public_id where id = '10000000-0000-0000-0000-000000000001'$$, 'Keeping existing public ID is harmless');
insert into schema_fixture_users(id) values ('10000000-0000-0000-0000-000000000005');
insert into auth.users(id) values ('10000000-0000-0000-0000-000000000005');
select is((select count(*) from profiles where id in (select id from schema_fixture_users)), 5::bigint, 'Auth signup automatically creates the fixture profile');
-- Isolate direct generation/constraint tests on an unused Auth identity.
...
select throws_ok($$insert into profiles(id, public_id) select '10000000-0000-0000-0000-000000000005', public_id from profiles where id = '10000000-0000-0000-0000-000000000001'$$, '23505', null, 'Public ID unique constraint rejects duplicates');
...
select throws_ok($$insert into profiles(id, public_id) select '10000000-0000-0000-0000-000000000005', lower(public_id::text)::citext from profiles where id = '10000000-0000-0000-0000-000000000001'$$, '23505', null, 'Uniqueness also rejects lowercase duplicate independently of format');
...
-- Auth deletion now cleans application data; 009_account_deletion.test.sql covers
-- that contract and its rollback. Keep these fixtures alive for the schema checks.
...
select throws_ok($$update automatic_target_states set set_id = -1 where pokemon_id = -1$$, '23514', null, 'Target state cannot have both targets');
select throws_ok($$update automatic_target_states set pokemon_id = null where pokemon_id = -1$$, '23514', null, 'Target state must have a target');
select throws_ok($$update automatic_target_states set target_type = 'set' where pokemon_id = -1$$, '23514', null, 'Target state type must match target');
select throws_ok($$update automatic_target_states set generation_version = 0 where pokemon_id = -1$$, '23514', null, 'Generation version positive');
select throws_ok($$update automatic_target_states set content_hash = '' where pokemon_id = -1$$, '23514', null, 'Hash nonempty');
...
select throws_ok($$insert into collections(owner_id, name, collection_type, automatic_target_type, target_pokemon_id, applied_target_version) values ('10000000-0000-0000-0000-000000000001', 'Same target rejected', 'automatic', 'pokemon', -1, 1)$$, '23505', null, 'One automatic Pokemon collection per owner and target');
...
select is((select count(*) from physical_copies where user_id in (select id from schema_fixture_users)), 4::bigint, 'Collection deletion preserves all fixture copies');
delete from collection_items where collection_id = '20000000-0000-0000-0000-000000000002' and variant_id = -2;
select throws_ok($$delete from catalog_variants where id = -2$$, '23503', null, 'Copy-only reference protects variant');
delete from physical_copies where user_id in (select id from schema_fixture_users) and variant_id = -1;
...
select is((select count(*) from collection_items where collection_id = '20000000-0000-0000-0000-000000000003' and variant_id = -1), 1::bigint, 'Inactivation does not remove item');
